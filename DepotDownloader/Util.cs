...
namespace DepotDownloader
{
    static class Util
    {
        ...
        /// <summary>
        /// Load the existing content of keys.txt and initialize the deduplication set
        /// </summary>
        public static void InitializeKeysFile()
        {
            try
            {
                lock (FileLock)
                {
                    if (File.Exists("keys.txt"))
                    {
                        ...
                        {
                            if (uint.TryParse(line.Split(';')[0], out uint depotId))
                            {
                                SavedDepotIds.Add(depotId);
                            }
                        }
                    }
                }
            ...
            {
                Console.WriteLine($"[WARN] Failed to load keys.txt: {ex.Message}");
            }
        }

        /// <summary>
        /// Safely append the DepotKey to keys.txt (automatically removing duplicates)
        /// </summary>
        public static void SaveDepotKeyToFile(uint depotId, byte[] depotKey)
        {
            try
            {
                lock (FileLock)
                {
                    // Check if the DepotID already exists
                    if (SavedDepotIds.Contains(depotId))
                    {
                        Console.WriteLine($"[INFO] DepotID {depotId} already exists. Skipping save.");
                        return;
                    }

                    // ToStr
                    string keyHex = BitConverter.ToString(depotKey).Replace("-", "");
                    string line = $"{depotId};{keyHex}";

                    // Append to files
                    File.AppendAllText("keys.txt", line + Environment.NewLine);
                    SavedDepotIds.Add(depotId); // Update the memory set

                    Console.WriteLine($"[INFO] Successfully saved DepotKey: {line}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] Failed to save DepotKey: {ex.Message}");
            }
        }
    }
}
